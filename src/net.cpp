...
static const char *strDNSSeed[][2] = {
	{"temp seed", "CRY.bottlecaps.org"},
	{"CCE block explorer", "CRY.altcointech.net"},
    {"chainworks seed", "CRY.chainworks.info"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Crypton is probably already running."), addrBind.ToString().c_str());
        else
